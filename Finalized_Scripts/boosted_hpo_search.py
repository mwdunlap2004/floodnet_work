...
def ensemble_predict(models: list, weights: list, X_tensor: torch.Tensor, batch_sz: int = 4096) -> np.ndarray:
    ...
    for m in models:
        ...
        m.cpu()  # Crucial: move back to system RAM immediately
    ...
    final_preds = sorted_preds[median_idx, np.arange(all_preds.shape[1])]
    return final_preds
...
def objective_ann(trial):
    ...
    for k in range(n_estimators):
        ...
        # Train this base learner for 15 epochs with current observation weights
        for _ in range(15):
            model.train()
            ...
            for i in range(0, n_samples, batch_sz):
                ...
                with autocast(device_type=PRIMARY.type):
                    loss = loss_fn(model(X_tr_gpu[idx]), y_tr_gpu[idx], batch_w)
                if scaler_amp:
        ...
        # Compute AdaBoost.R2 terms for this learner
        model.eval()
        with torch.no_grad(), autocast(device_type=PRIMARY.type):
            ...
            w       = w * (beta_k ** (1.0 - L_i))
            w       = w / torch.sum(w)
            alpha_k = torch.log(1.0 / beta_k).item()
            models.append(model.cpu())
    ...
    val_np_preds = ensemble_predict(models, model_weights, X_val_gpu, batch_sz)
    tr_np_preds  = ensemble_predict(models, model_weights, X_tr_gpu,  batch_sz)

    epoch_kge    = kge(y_val_raw, val_np_preds)
    epoch_nse    = nse(y_val_raw_gpu,
                       torch.tensor(val_np_preds, device=PRIMARY))
    ...
    epoch_tr_nse = nse(
        ...
        torch.tensor(tr_np_preds, device=PRIMARY)
    )
...
def objective_lstm(trial):
    ...
    try:
        ...
        # Collect windowed predictions from each ensemble member
        all_val_preds = []
        all_tr_preds  = []
        for m in models:
            m.to(PRIMARY)
            ...
            with torch.no_grad():
                ...
                for j in range(0, len(Xvw_cpu), batch_sz):
                    vp.append(_descale(m(Xvw_cpu[j:j+batch_sz].to(PRIMARY))).cpu().numpy().flatten())
                for j in range(0, len(Xtw_cpu), batch_sz):
                    tp.append(_descale(m(Xtw_cpu[j:j+batch_sz].to(PRIMARY))).cpu().numpy().flatten())
            all_val_preds.append(np.concatenate(vp))
        ...
        def _weighted_median(preds_list):
            ...
            sidx = np.argsort(arr, axis=0)
            sarr = np.take_along_axis(arr, sidx, axis=0)
            sw   = weights_arr[sidx]
            cw   = np.cumsum(sw, axis=0)
            half = 0.5 * weights_arr.sum()
            midx = np.argmax(cw >= half, axis=0)
            return sarr[midx, np.arange(arr.shape[1])]

        val_pred_np = _weighted_median(all_val_preds)
        tr_pred_np  = _weighted_median(all_tr_preds)
